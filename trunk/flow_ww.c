...
char emname[30]="rstart.dat";         // emergency points
...
s      GA_Sync();
  ...
  /* write weights */
	...
	for (pt=0; pt<limit; pt++) {
	  ...
	  //LINE_STAMP;
	  GetFromServer_int(&Coords_from,0,ind,tempi);
	  //LINE_STAMP;
	  GetFromServer_int(&Coords_from,0,ind+1,tempi2);
	  fprintf(outFile,"%d %d ", tempi[0], tempi2[0]);

	  ind = mxlist*beads*j + mxlist*k + pt;
	  //LINE_STAMP;
	  GetFromServer_dbl(&Coords_wlist,0,ind,tempw);
	  GetFromServer_int(&Coords_clock,0,ind,tempi);
	  fprintf(outFile,"%e %d ",tempw[0],tempi[0]);
	  GetFromServer_int(&Coords_mytim,0,ind,tempi);
...
int back (int dir, int bead) {
  ...
  int i, j, newreg, ind, limit, k, mcount, tfrom2[2], gotit;
  ...
  while (!good) {
	...
	tempw[0] = sum;
	ind = dir*beads + bead;
	PutToServer_dbl(&Coords_twlist,0,ind,tempw);
	ttwlist = tempw[0];
      }
	...
	if (myweight < 0) {
	  printf("proc %d: Error! myweight = %f (dir,bead = %d,%d)\n",me,myweight,dir,bead);
	  gaexit(20);
	}
	
	PutToServer_dbl(&Coords_wlist,0,wind,tempw);
	GetFromServer_dbl(&Coords_twlist,0,ind,tempw);
	tempw[0] -= myweight;
	PutToServer_dbl(&Coords_twlist,0,ind,tempw);
	if (verb >=4) {
	  t2 = MPI_Wtime();
	  mainout = fopen(outname,"a");
	  fprintf(mainout,"%f: Subtracting weight from (%d,%d):  %e - %e = %e\n",t2,dir,bead,tempw[0]+myweight,myweight,tempw[0]); 
	  fclose(mainout);
	}
	sum = 0.;
	for (i=0;i<limit;i++) {
...
void addpoint (int dir, int bead, point_t x, double wt, int from1, int from2) {
  ...
  int tgt, n, i, ind, tfull, wind, part, dim, ptind, mcount, tn, tfrom2[2];
  ...
  if (tn == mxlist+1) {
    tempi[0] = 1;
    PutToServer_int(&Coords_nlist,0,ind,tempi);
    tn = 1;
    if (!tfull) {   /* not full */
      tempi2[0] = 1;
      PutToServer_int(&Coords_full,0,ind,tempi2);
      tfull = 1;
    }
  }
  GA_Fence();
  ...
  if (tfull) {
    ...
    GetFromServer_dbl(&Coords_twlist,0,ind,ttw);
    ttw[0] += wt;
    PutToServer_dbl(&Coords_twlist,0,ind,ttw);
    if (verb >=4) {
      t2 = MPI_Wtime();
      mainout = fopen(outname,"a");
      fprintf(mainout,"%f: Adding weight to (%d,%d):  %e + %e = %e\n",t2,dir,bead,ttw[0]-wt,wt,ttw[0]); 
      fclose(mainout);
    }
    ...
    tw[0] = wt;
    PutToServer_dbl(&Coords_wlist,0,wind,tw);
    //flist[dir][bead].wlist[n] = wt;
  ...
  tempi[0] = myclock;
  PutToServer_int(&Coords_clock,0,ind,tempi);
  tempi[0] = mytim;
  PutToServer_int(&Coords_mytim,0,ind,tempi);
...
void fxread() {
  ...
  int j, k, op, dir, ti, tj, tk, limit, pt, dim, part, ind, mcount;
  FILE * fxin;
  float temp;
  float tw;
  ...
  for (j=0; j<=1; j++) {
	...
	ind = 2*mxlist*beads*j + 2*mxlist*k + 2*pt;
	PutToServer_int(&Coords_from,0,ind,tempi);
	PutToServer_int(&Coords_from,0,ind+1,tempi2);
	    
	fscanf(fxin,"%f %d ",&tw,&tempi[0]);
	tempw[0] = tw;
	ind = mxlist*beads*j + mxlist*k + pt;
	PutToServer_dbl(&Coords_wlist,0,ind,tempw);
	PutToServer_int(&Coords_clock,0,ind,tempi);
...
void wrtflux(int find, int cycle) {
  ...
  int j,k,pt,part,dim,mcount,ind,limit,scale;
  double extra, fac;
