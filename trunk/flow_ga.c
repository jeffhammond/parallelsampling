...
void dostep(){
  ...
  for(i=0; i< npart; i++){
    for(x=0; x< 3; x++){
      ...
      f2[i][x] = f.x[i][x];
      coor.v[i][x] += (f1[i][x]+f2[i][x])/(2.)*tstep;
      f1[i][x] = f2[i][x];
    }
  }
}
...
void rdemerg(){
  ...
  for (j=0;j<nemerg;j++) {
	...
	emerg[j].x[part][dim] = atof(stemp);
      ...
      for (dim=0;dim<ndim;dim++) {
	emerg[j].v[part][dim] = sqrt(-var2*2.*log(ran2(&seed)))*cos(2*Pi*ran2(&seed));
      }
    }
  }
  fclose(filein);
...
void dorotate() {
  ...
  double deltax, deltay, deltaz, rho, psi, temp1, rndvec[ndim], odisp[ndim];
  int i, j, k, l, m, n, good, rr, ind, ii;
  double temp4;
  ...
  while (!good) {
    ...
    for(i=0;i<N;i++){
      if(rsolv[i][0]>deltax){
	pos[i][0]=(int)(blinv*rsolv[i][0]-deltax)+1;
	if (pos[i][0] == nx) {
	  pos[i][0] = 0;
	}
      } else {
	pos[i][0]=0;
      ...
      j = pos[i][0];
      if(rsolv[i][1]>deltay){
	pos[i][1]=(int)(blinv*rsolv[i][1]-deltay)+1;
      } else {
	pos[i][1]=0;
      }
      k = pos[i][1];
      if(rsolv[i][2]>deltaz){
	pos[i][2]=(int)(blinv*rsolv[i][2]-deltaz)+1;
	if (pos[i][2] == nz) {
	  pos[i][2] = 0;
	}
      } else {
  ...
  for(i=0;i<npart;i++){
    if(coor.x[i][0]>deltax){
      pos_poly[i][0]=(int)(blinv*coor.x[i][0]-deltax)+1;
    } else {
      pos_poly[i][0]=0;
      if (pos_poly[i][0] < 0) {
	printf("Polymer outside of box! x[%d] = %e\n",i,pos_poly[i][0]);	      
	gaexit(2);
      }
    }
    if(coor.x[i][1]>deltay){
      pos_poly[i][1]=(int)(blinv*coor.x[i][1]-deltay)+1;
    } else {    
      pos_poly[i][1]=0;
      if (pos_poly[i][1] < 0) {
	printf("Polymer outside of box! y[%d] = %e\n",i,pos_poly[i][1]);	      
	gaexit(2);
      }
    }
    if(coor.x[i][2]>deltaz){
      pos_poly[i][2]=(int)(blinv*coor.x[i][2]-deltaz)+1;
    } else {
      pos_poly[i][2]=0;
      if (pos_poly[i][2] < 0) {
	printf("Polymer outside of box! z[%d] = %e\n",i,pos_poly[i][2]);	      
	gaexit(2);
      }
    }
  }
  ...
  for(i=0;i<nx;i++){
	...
	temp4=sqrt(-var*2.*log(ran2(&seed)))*cos(2*Pi*ran2(&seed));
